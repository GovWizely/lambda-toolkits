...
var CN = {
    ...
    return new Promise((resolve) => {
      ...
      _.forEach(filterMapByType.Product, (product) => {
        _.forEach(product.relatedParticipantIds, (participantId) => {
          ...
          product.addId(PROVIDER_ID, provider.getIds(PROVIDER_ID))
          provider.addId(PRODUCT_ID, product.getIds(PRODUCT_ID))
        })
      })
  ...
  _buildSectors: (filterMapByType) => {
    ...
    return new Promise((resolve) => {
      let sectorNames = _.map(filterMapByType.Product, (product) => {
        return product.relatedFilters.Sector
      })
      ...
      _.forEach(filterMapByType.Product, (product) => {
        _.forEach(product.relatedFilters.Sector, (sectorName) => {
          let sector = sectorMap[sectorName]
          product.addId(SECTOR_ID, sector.getIds(SECTOR_ID))
          sector.addId(PRODUCT_ID, product.getIds(PRODUCT_ID))
        })
      })
  ...
  _buildSubSectors: (filterMapByType) => {
    ...
    let index = 0
    _.forEach(filterMapByType.Product, (product) => {
      _.forEach(product.relatedFilters['Sub-Sector'], (sectorName) => {
        ...
        if (_.isNil(subSector)) {
          ...
          subSectorId = subSector.getIds(SUB_SECTOR_ID)
        }
        product.addId(SUB_SECTOR_ID, subSectorId)
        subSector.addId(PRODUCT_ID, product.getIds(PRODUCT_ID))
  ...
  _updateRelatedFilterIds: (filterMapByType) => {
    ...
    _.forEach(filterMapByType.Product, (product) => {
      ...
      productMap[product.getIds(PRODUCT_ID)[0]] = product
    })

    _.forEach(filterMapByType.Provider, (provider) => {
      _.forEach(provider.getIds(PRODUCT_ID), (productId) => {
        let product = productMap[productId]
        provider.addId(SECTOR_ID, product.getIds(SECTOR_ID))
        provider.addId(SUB_SECTOR_ID, product.getIds(SUB_SECTOR_ID))
      })
    })

    _.forEach(filterMapByType.Sector, (sector) => {
      _.forEach(sector.getIds(PRODUCT_ID), (productId) => {
        let product = productMap[productId]
        sector.addId(PROVIDER_ID, product.getIds(PROVIDER_ID))
        sector.addId(SUB_SECTOR_ID, product.getIds(SUB_SECTOR_ID))
      })
    })

    _.forEach(filterMapByType['Sub-Sector'], (subSector) => {
      _.forEach(subSector.getIds(PRODUCT_ID), (productId) => {
        let product = productMap[productId]
        subSector.addId(PROVIDER_ID, product.getIds(PROVIDER_ID))
        subSector.addId(SECTOR_ID, product.getIds(SECTOR_ID))
      })
    })
  },
